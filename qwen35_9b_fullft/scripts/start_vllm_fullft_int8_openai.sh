...
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen35-9b-fullft-int8}"
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-8000}"
...
LOG_FILE="${LOG_DIR}/vllm_fullft_int8_${TS}.log"
PID_FILE="${LOG_DIR}/vllm_fullft_int8.pid"
...
# vLLM expects Qwen3.5 multimodal wrapper config (`qwen3_5`) even in text-only
# mode. If the training export is text-only (`qwen3_5_text`), create a
# compatible serving copy that rewrites config only and keeps weight files.
...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -n "${OLD_PID}" ]] && kill -0 "${OLD_PID}" 2>/dev/null; then
    ...
    echo "Stop first: ${SCRIPT_DIR}/stop_vllm_fullft_openai.sh"
    exit 0
  fi
fi
