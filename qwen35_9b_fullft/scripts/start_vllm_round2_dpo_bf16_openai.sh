...
MODEL_PATH_DEFAULT="${PROJECT_DIR}/runs/20260421_032308_qwen35_9b_round2_dpo_702_clean_16k_v1/artifacts/full_model"
MODEL_PATH="${MODEL_PATH:-$MODEL_PATH_DEFAULT}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen35-9b-round2-dpo-bf16}"
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-8002}"
...
LOG_FILE="${LOG_DIR}/vllm_round2_dpo_bf16_${TS}.log"
PID_FILE="${LOG_DIR}/vllm_round2_dpo_bf16.pid"
...
if [[ -f "${PID_FILE}" ]]; then
  ...
  if [[ -n "${OLD_PID}" ]] && kill -0 "${OLD_PID}" 2>/dev/null; then
    ...
    echo "Stop first: ${SCRIPT_DIR}/stop_vllm_round2_dpo_bf16_openai.sh"
    exit 0
  fi
fi
