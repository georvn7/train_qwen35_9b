...
LOG_DIR="$WORK/logs"
RUN_LOG="$LOG_DIR/train_qwen35_9b_qlora32k_resume_safe.log"
FAIL_LEDGER="$LOG_DIR/train_qwen35_9b_qlora32k_failed_steps.tsv"
FAILED_STEPS_TODO="$STATE_DIR/train_qwen35_9b_qlora32k_failed_steps_todo.txt"
SESSION_PTR="$STATE_DIR/session_qwen35_9b_qlora32k_e1.txt"

SAVE_STEPS="${SAVE_STEPS:-10}"
LOGGING_STEPS="${LOGGING_STEPS:-5}"
MAX_GPU_MEMORY_GIB="${MAX_GPU_MEMORY_GIB:-112}"
...
LABEL="${LABEL:-candidate_qwen35_9b_qlora32k_all1109_seed3413_e1}"
...
resolve_session_dir() {
  ...
  "$VENV_PY" "$WORK/qwen35_9b_fullft/scripts/create_session.py" \
    ...
    --label "$LABEL" \
    --notes "Qwen3.5-9B QLoRA-32K on local all_1109_rows_no_assistant_thinking.jsonl (resume-safe)" \
    >> "$RUN_LOG" 2>&1
...
build_train_cmd() {
  ...
  CMD=(
    ...
    --bnb-4bit-compute-dtype "${BNB_4BIT_COMPUTE_DTYPE:-bfloat16}"
...
while true; do
  ...
  if [[ "${new_ckpt:-}" != "${CKPT:-}" ]]; then
    ...
    record_failure "train_session" "$total_attempts" "$stagnant_failures" "$rc" "$CKPT" "$new_ckpt" "failed_with_progress_retrying"
    stagnant_failures=0
    last_ckpt="${new_ckpt:-}"
  else
    ...
    record_failure "train_session" "$total_attempts" "$stagnant_failures" "$rc" "$CKPT" "$new_ckpt" "failed_no_progress"
  fi
